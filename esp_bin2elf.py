...
def name_sections(rom):
    ...
    print(b" ".join(default_section_settings.keys()))
    print("if defaults are unavailable for a name, generic values will be used.")

    for section in rom.sections:
        name = bytes(input("enter a name for 0x%04x> " % (section.address)), 'utf-8')
...
def convert_rom_to_elf(esp_rom, addr_to_section_name_mapping, filename_to_write=None):
    ...
    bootrom_section = ElfSection(b'.bootrom.text', 0x40000000, bootrom_bytes)
    ...
    for name, addr in symbols.items():
        elf.add_symbol(name, addr, b'.bootrom.text')
